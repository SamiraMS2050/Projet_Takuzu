...
ui <- fluidPage(
  ...
  div(
    ...
    div(
      ...
      actionButton("easy", "Facile (6×6)", class = "difficulty-btn"),
      actionButton("medium", "Moyen (8×8)", class = "difficulty-btn"),
      actionButton("hard", "Difficile (10×10)", class = "difficulty-btn")
    ),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$hard, {
    difficulty("hard")
    size(10)  # Grille 10x10 pour niveau difficile
    shinyjs::removeClass("easy", "selected-difficulty")
    shinyjs::removeClass("medium", "selected-difficulty")
  ...
  # Fonction pour obtenir le taux de remplissage (maintenant fixé à 0.4 pour tous les niveaux)
  getFillRate <- function() {
    return(0.4)  # Taux de remplissage fixé à 40% pour tous les niveaux
  }
  ...
  initializeGame <- function() {
    ...
    output$result <- renderText(paste("Nouvelle partie - Difficulté:", difficulty()))
  }
  ...
  output$takuzu_ui <- renderUI({
    ...
    do.call(fluidRow, lapply(cells, function(row) {
      column(12, do.call(splitLayout, c(row, list(cellWidths = rep("42px", current_size)))))
    }))
  })
  
  # Gestion des clics sur les cellules (doit être dynamique en fonction de la taille)
  observeEvent(takuzu_grid(), {
    current_size <- size()
    grid <- takuzu_grid()
    if (is.null(grid)) return()
    
    # On réinitialise les observateurs pour les cellules
    lapply(1:100, function(i) { # Utilisation d'un nombre suffisamment grand pour couvrir toutes les tailles potentielles
      row <- ceiling(i / 10)
      col <- i %% 10
      if (col == 0) col <- 10
      ...
      if (row <= current_size && col <= current_size) {
        # Uniquement pour les cellules dans la grille actuelle
